...
size, etime, opers = loadtxt("timeDivideAndConquer3D.dat").T
...
c1 = ( size * log(size)**2 ).mean() / etime.mean()
...
ax.loglog(size, size, color='black', linestyle='--',
          label='linear')
ax.loglog(size, size * log(size)**2, color='black', linestyle='--',
          label='super-linear')
# plots the scaled elapsed time as a function of size
ax.loglog(size, c1 * etime, color='black',
    linestyle='', marker='o', markersize=12, label='elapsed-times')
# plots the scaled number of operations as a function of size
