...
% f is the forcing constant, and u(t) = P is the impulse-input function of
% magnitude P.
%
%
...
% defines inertia, friction, stiffness, forcing, and impulse magnitude
[m, b, k, f, P] = prms();
...
% plots the analytic solution of the impulse response
t = t_Euler;
plot(t, f(t), '-k', 'DisplayName', 'analytic')
hold on
% plots Euler's method solution
plot(t_Euler(1:128:end), y2_Euler(1:128:end), 'linestyle', 'none', ...
...
plot(t_RK(1:14:end), y2_RK(1:14:end), 'linestyle', 'none', ...
...
function [m, b, k, f, P] = prms()
%   Synopsis:
%   Returns the mass (or moment of inertia), friction, stiffness,
%   forcing-constant, and the impulse magnitude, respectively.

    m = 0.2;	b = 1.6;	k = 65.0;	f = 1.0;	P = 2.5;
end


function P = u(t)
%   Synopsis:
%   Defines the input impulse function u(t)
    [~, ~, ~, ~, P] = prms();
end
...
%   dynamic mechanical system given the system mass, friction and stiffness
%   constants, the forcing constant, and magnitude P of the impulse u(t).
    ...
    % scales to obtain the system response to an impulse of magnitude P
    y = (b0 * P / omega^2) * y;
