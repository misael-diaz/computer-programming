...
public class Ensemble	// Particle Ensemble Class
{
	...
	public static Tree genDataSet (int size)
	/*

	Synopsis:
	Generates a sorted dataset of 2D Cartesian coordinates by sampling
	values from a uniform pseudo-random number generator PRNG.

	Inputs:
	size		size of the data set (number of particles)

	Output:
	tree		a tree that stores the data set of coordinates

	*/
	{
		// creates a tree for storing the coordinates
		Tree tree = new Tree();
		...
		// generates the distinct set of (x, y) coordinates
		{
			...
			// generates a new coordinate if already in tree
			{
				x = random.nextInt(size);
				y = random.nextInt(size);
				c = new Coord (x, y);
			}
			// inserts (distinct) coordinate in the tree
			tree.insert(c);
		}
	...
	/* Distance Computing and Brute Force Methods */


	private static double distance (Tree part, int i, int j)
	/*
	...
	*/
	...
	private static double distance (Tree L, Tree R, int i, int j)
	/*
	...
	*/
	...
	private static double distance (Tree part, Coord [] closestPair)
	/*
	...
	*/
	...
	private static double distance (Tree L, Tree R, Coord [] pair,
	...
	private static Tree [] divide (Tree Px)
	/*
	...
	*/
	...
	private static double combine (
		Tree Lx, Tree Rx, Coord [] closestPair, double d_min
	)
	/*
	...
	*/
	...
	private static double recurse (Tree Px, Coord [] closestPair)
	/*
	...
	*/
	{
		...
		else
		{
			...
			Tree [] partitions = divide (Px);
			// gets left and right partitions
			Tree Lx = partitions[0], Rx = partitions[1];
	...
	public static double recursiveClosestPair (Tree data)
	/*
	...
	*/
	...
	public static double bruteforce (Tree data)
	/*
	...
	*/
	...
	Synopsis:
	Tests the implementation of the Brute Force and Divide and Conquer
	Algorithms that solve the Closest Pair problem. If the squared
	distance returned by the algorithms do not match the test throws
	a runtime exception to inform the user that there is an error in
	the implementation. Note that the algorithms may find different
	instances of the closest pair but their distances must be equal.

	*/
	{
		// defines the initial system size
		int size = (0x00000010);
		// defines the number of experiments
		int runs = (0x00000010);
		for (int i = 0; i != runs; ++i)
		{
			// stores the dataset of coordinates in tree
			Tree t = genDataSet (size);
			// finds the closest pair with brute force
			double minDistBruteForce = bruteforce (t);
			// finds the closest pair recursively
			double minDistRecursive = recursiveClosestPair (t);

			if (minDistBruteForce != minDistRecursive)
			// complains if the outputs do not match
			{
				String errmsg = ("ImplementationError");
				throw new RuntimeException(errmsg);
			}

			// doubles the system size
			size *= 2;
		}
	}
}
